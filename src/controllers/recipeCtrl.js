...
export default class Recipe {
  ...
  postShareRecipe() {
    ...
    let { meal, ingredients, prepMethod, description, foodImageURL, category } =
      this.params;
    formData = {
      meal,
      ingredients,
    ...
    if (isValid) {
      ...
      const titleIngredients = formData.ingredients.filter(
        (item, index) => index < 4
      );
      ...
      //server request
      db.post("Recipes", serverData, sessionStorage.getItem("loggedIn")).then(
        (res) => {
  ...
  getDetails() {
    ...
    const viewRecipe = allRecipes.find(
    ...
    viewRecipe.likes.length == 1
      ? (hasOnlyOneLike = true)
      : (hasOnlyOneLike = false);
    ...
    let likeButton;
    let editButtons;
    viewRecipe.user === sharedData.activeUserId
      ? (likeButton = true)
      : (likeButton = false);
    viewRecipe.user === sharedData.activeUserId
      ? (editButtons = false)
  ...
  getLike() {
    ...
    if (allRecipes[index].likes.includes(userId)) {
      ...
      allRecipes[index].likes = allRecipes[index].likes.filter(
        (id) => id != userId
      );
      msgs.push({
    ...
    db.edit("Recipes", id, viewData, sessionStorage.getItem("loggedIn")).then(
      (res) => {
